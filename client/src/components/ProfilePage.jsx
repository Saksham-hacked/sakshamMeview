...
export default function ProfilePage({ currUser, setUserLoading}) {
  ...
  useEffect(() => {
    const fetchAllUserData = async () => {
      ...
      // 1. Check if user data is already in props
      if (currUser) {
        ...
        try {
          ...
          const data = await response.json();
          fetchedUserData = data.data;
          setUserData(fetchedUserData);
        } catch (error) {
          console.error("Error fetching user data:", error);
          setLoading(false);
          // Redirect to signin if fetching fails (e.g., session expired)
          navigate("/user/signin");
          return;
        }
      }
      ...
      if (fetchedUserData) {
        try {
          // Fetch user reviews
          const reviewsResponse = await fetch(`${envApi}/review/userReview/${fetchedUserData._id}`, {
            method: "GET",
            credentials: "include",
            headers: { "Content-Type": "application/json" },
          });
          ...
          // Fetch user lists
          const listsResponse = await fetch(`${envApi}/topfive/user/${fetchedUserData._id}`, {
            method: "GET",
            credentials: "include",
            headers: { "Content-Type": "application/json" },
          });
          const listsData = await listsResponse.json();
          setUserLists(Array.isArray(listsData.data) ? listsData.data : []);
  ...
  const handleReviewSubmit = async () => {
    ...
    try {
      const response = await fetch(`${envApi}/review/add`, {
        method: "POST",
        credentials: "include",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({
      ...
      setUserReviews(prevReviews => [newReview.data, ...prevReviews]);
  ...
  const handleCreateListSubmit = async () => {
    ...
    try {
      const response = await fetch(`${envApi}/topfive/create`, {
        method: "POST",
        credentials: "include",
        headers: { "Content-Type": "application/json" },
      ...
      console.log("Submitting list:", {
       ...
       if (!response.ok) {
      // If not, parse the error message from the server response
      const errorData = await response.json();
      throw new Error(errorData.message || "Failed to create list.");
    }
      const newList = await response.json();
      setUserLists(prevLists => [newList.data, ...prevLists]);
              ...
              <div className="p-6 space-y-6 max-h-[500px] overflow-y-auto custom-scrollbar">
                {userReviews.length > 0 ? (
                  userReviews.map((r) => (
                    <div
  key={r._id}
  className="relative flex flex-col sm:flex-row items-start gap-4 sm:gap-6 p-4 sm:p-6 bg-zinc-800 rounded-xl shadow-md hover:ring-2 hover:ring-yellow-400/30 transition-all duration-300"
>
  {/* Movie Poster */}
  <img
    src={`https://image.tmdb.org/t/p/w92${r.moviePoster}`}
    alt={r.movieTitle}
    className="w-20 h-28 object-cover rounded-lg flex-shrink-0 mb-3 sm:mb-0"
  />

  {/* Content area */}
  <div className="flex-1 flex flex-col">
    {/* Title */}
    <h3 className="font-bold text-gray-100 text-lg truncate mb-1">{r.movieTitle}</h3>
  ...
  <button
    onClick={() => handleDeleteReview(r._id)}
    className="absolute top-3 right-3 p-2 bg-black/40 text-red-400 rounded-full hover:bg-red-500/80 hover:text-white transition-colors"
    title="Delete Review"
    aria-label="Delete Review"
  >
    <Trash2 className="h-5 w-5" />
